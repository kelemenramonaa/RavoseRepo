...
class RoutineEveningActivity : AppCompatActivity() {
    ...
    private fun generateSmartRoutine() {
        lifecycleScope.launch {
            ...
            if (generatedTasks.isEmpty()) {
                runOnUiThread { Toast.makeText(this@RoutineEveningActivity, "Nincs megfelelő termék a polcodon!", Toast.LENGTH_SHORT).show() }
            } else {
                ...
                runOnUiThread {
                    ...
                    btnComplete.visibility = View.VISIBLE
                    Toast.makeText(this@RoutineEveningActivity, "Rutin generálva!", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
    ...
    private fun showProductSelector() {
        lifecycleScope.launch {
            ...
            if (displayNames.isEmpty()) {
                runOnUiThread { Toast.makeText(this@RoutineEveningActivity, "Még nincsenek termékeid!", Toast.LENGTH_SHORT).show() }
                return@launch
            }

            runOnUiThread {
                AlertDialog.Builder(this@RoutineEveningActivity)
    ...
    private fun refreshUI() {
        ...
        tvSubtitle.text = "${currentRoutineTasks.size} lépés • ${currentRoutineTasks.size * 3} perc összesen"
    ...
    private fun saveRoutine() {
        ...
        prefs.edit().putString(KEY_EVENING_ROUTINE, jsonArray.toString()).apply()
    }
}
